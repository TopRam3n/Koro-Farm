...
export function DemoControls({ requirementId, allocations, hasPlan, recoveryStatus }: { requirementId: string; allocations: Allocation[]; hasPlan: boolean; recoveryStatus?: string }) {
  ...
  return <Card className="border-primary/20 bg-primary/[0.04]"><CardHeader className="pb-3"><p className="section-eyebrow">Guided demonstration</p><CardTitle className="mt-2 text-lg">Programme controls</CardTitle></CardHeader><CardContent className="space-y-3">
    ...
    {hasPlan && committed && !recoveryStatus && <Button variant="outline" disabled={busy} onClick={() => dropout.mutate({ allocationId: committed.id, reason: 'demo_farmer_dropout' })}><AlertTriangle className="mr-2 h-4 w-4" />Simulate {Number(committed.quantity_kg).toLocaleString('en-JM')} kg dropout</Button>}
    {recoveryStatus === 'running' && <Button disabled={busy} onClick={() => recovery.mutate()}><Play className="mr-2 h-4 w-4" />Run recovery plan</Button>}
